...
async fn enqueue_file_blocking_budget_gate_fallback() {
    ...
    assert_eq!(
        stats.enqueued.load(Ordering::Relaxed),
        0,
        "budget fallback does not queue an item"
    );
    assert!(
        source.exists(),
...
#[tokio::test]
async fn enqueue_file_blocking_closed_channel_rolls_back_enqueued() {
    let temp = tempfile::TempDir::new().unwrap();
    let queue_dir = temp.path().join("upload_queue");
    std::fs::create_dir_all(&queue_dir).unwrap();
    let source = temp.path().join("src.bin");
    std::fs::write(&source, vec![0xCD; 128]).unwrap();
    let stats = Arc::new(UploadQueueStats::new());
    let (tx, rx) = mpsc::channel(CHANNEL_CAPACITY);
    drop(rx);
    let queue = UploadQueue {
    ...
    let err = queue
        ...
        .await;
    let err = match err {
        Ok(_) => panic!("closed worker should return Err"),
        Err(err) => err,
    };
    assert!(
        err.to_string().contains("Upload queue closed"),
        "error should surface closed worker"
    );
    assert_eq!(stats.pending.load(Ordering::Relaxed), 0);
    assert_eq!(stats.pending_bytes.load(Ordering::Relaxed), 0);
    assert_eq!(
        stats.enqueued.load(Ordering::Relaxed),
        0,
        "closed channel must roll back enqueued counter"
    );
    let staged: Vec<_> = std::fs::read_dir(&queue_dir).unwrap().flatten().collect();
    assert!(
        staged.is_empty(),
        "rejected staged copy must be removed on closed channel"
    );
}
...
async fn enqueue_file_channel_full_streams_from_source_path() {
    ...
    assert_eq!(
        stats.enqueued.load(Ordering::Relaxed),
        0,
        "channel-full fallback must roll back enqueued"
    );
...
async fn enqueue_file_reference_channel_closed_falls_back_inline() {
    ...
    assert_eq!(
        stats.enqueued.load(Ordering::Relaxed),
        0,
        "closed-channel fallback must roll back enqueued"
    );
}
...
async fn enqueue_file_reference_channel_full_falls_back_inline() {
    ...
    assert_eq!(
        stats.enqueued.load(Ordering::Relaxed),
        0,
        "full-channel fallback must roll back enqueued"
    );
    assert!(source.exists(), "source preserved");
